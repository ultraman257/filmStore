...
router.get('/list', function (req, res, next) {
  ...
  // Get films
  r.table('films').filter(filter).then(async result => {
...
router.post('/seen', (req, res) => {
  ...
  r.table('films').get(req.body.id).update({ watched: req.body.watched }).then(result => {
    res.json({ message: "success" });
  }).catch(error => {
    return res.json({ message: "failed", reason: "an error occured when writing to the database."});
  })
...
router.post('/add', (req, res) => {
  ...
  if(!req.body) return res.json({ message: "failed", reason: "body is missing"});
  ...
  r.table('films').insert(newFilm).then(result => {
...
router.get('/search', function(req, res) {
  ...
  axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${process.env.TMDB_KEY}&query=${req.query.film}`, {}).then(result => {
    return {films: result.data, ...result.status}
  }).then(result => {
    console.log(result);
    res.json({ message: "success", query: req.query.filmName,  matches: result.films.results.slice(0, 5)});
  });
